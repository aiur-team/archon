...
function mountSharePanel(event) {
   ...
   * tier it wrote. What the panel reports now is the document's own domain list:
   * a read-only line, for the owner and for everybody else alike.
   *
   * It is deliberately not editable yet. The list is the document's access
   * policy and deserves a real editor -- entries, validation, the reason a
   * mailbox provider is refused -- which is ACN-009's panel. Shipping a select
   * that posted the retired body would be a control that fails on every click,
   * so it is gone rather than left pointing at a route that no longer answers.
   */

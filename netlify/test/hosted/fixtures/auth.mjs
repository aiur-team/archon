...
import { GITHUB_TOKEN_URL, GITHUB_USER_URL } from "../../../lib/hosted/github-oauth.mjs";
...
/**
 * The three synthetic accounts, as C1 v2 subjects.
 *
 * The relationships they encode are the point and are unchanged: two distinct
 * accounts, one of which is seen twice under two different display names. What
 * changed is what carries the identity - the derived `accountId`, not the raw
 * id - and the account identifiers below are literals rather than computed
 * values, because `validatePrincipal` recomputes the digest from the subject and
 * a mistyped literal is therefore a loud failure in every suite that uses it.
 *
 * `github|1010` is also the subject `githubProvider` below produces, so the
 * fixture and the real code exchange name one account.
 */

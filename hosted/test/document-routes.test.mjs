...
import { validateWireError } from "../lib/contracts.mjs";
import { SESSION_COOKIE } from "../lib/identity.mjs";
import readHandler, {
  createDocumentReadRoutes,
  config as readConfig,
} from "../functions/document-read.mjs";
import viewerHandler, {
  createViewerRoute,
  config as viewerConfig,
} from "../functions/document-viewer.mjs";
...
test("a session-store outage is 503, never a redirect and never a denial", async () => {
  const failing = {
    async readSession() {
      const { AuthUnavailableError } = await import("../lib/auth-errors.mjs");

...
- Execution profile: 13 work units in six lanes and seven dependency levels; local producer tests prove every unit, and a deployed acceptance capstone proves the two hostnames, the Auth0 round trip, and the domain gate on the live site.
...
- R21. Access by domain is evaluated on every request against the verified email carried by the current session; no per-person grant is stored when a domain admits someone, and a session lasts at most 24 hours, so a changed or removed email loses access within a day.
...
- Not provisioning sign-in for self-hosted sites produced by `scripts/connect.mjs`; they keep serving and configure their own Auth0 tenant if they want sign-in.
...
Level 1: U3. Level 2: U1, U4. Level 3: U2, U5, U7, U10. Level 4: U6. Level 5: U8, U11. Level 6: U9, U12. Level 7: U13. Levels are the longest dependency path from the unit table; U1 and U3 both edit `netlify.toml` and `check.yml`, so U1 depends on U3 rather than running beside it.
...
- **Approach:** Keep `templates/build --site` as the build command and `_site` as the publish directory. In `site.ts`, after the generated pages, copy `netlify/public/` (the former `hosted/public`) into `_site/`, copy `skills/` and the root `AGENTS.md` and `llms.txt` into `_site/`, and invoke the renderer build with an output directory of `_site/_render/`. The `netlify/public/` copy runs after the root `login`/`invite` copy and wins on collision. Extend the reserved top-level route set with `docs`, `publish`, `skills`, `_render`, `viewer.js`, `viewer.css`. Remove `_headers` from the renderer build's output and inventory assertions; the edge gate is the only renderer header authority. Add a pre-flight script step that fetches the site's `*.netlify.app` hostname and fails the build if it answers with a redirect. Move the `/publish/authorize` rewrite into the generated `_redirects`. Delete the root `X-Frame-Options` and the hosted `/*` CSP from TOML; headers move to U2. Rewrite the three CI assertions that pin the old TOMLs to assert the merged shape (one edge function on `/*`, no `X-Frame-Options` header rule, no second TOML). Keep the connect manifest at four entries and extend `scripts/connect.test.mjs` with a case proving a vendored tree with no `HOSTED_*` variables still serves `/` and `/<slug>/`.
...
- **Test scenarios:**
  - Happy path: a site build emits `_site/_render/index.html`, `renderer.js`, `renderer.css`, `renderer-config.js`, `_site/skills/archon-doc/SKILL.md`, `_site/AGENTS.md`, `_site/llms.txt`, `_site/publish/authorize.html`, `_site/viewer.js`, `_site/login/index.html` (the hosted copy).
  ...
  - Integration: `scripts/check-test-inventory.mjs`, `scripts/vendor-netlify-lib.mjs`, and `templates/check-dist` pass on the merged tree; `_site/_render/` holds exactly the four shell files.
...
- **Dependencies:** U1.
- **Files:** `netlify/edge-functions/gate.ts`, `netlify/edge-functions/host.mjs` (new: host classification, refusal matrix, header sets; plain ESM so `node --test` runs it), `netlify/edge-functions/host.test.mjs` (new), `netlify.toml` (drop `excludedPath`), `.github/workflows/check.yml`, `scripts/test-hosted-renderer.mjs`, `scripts/connect.test.mjs`.
- **Approach:** Read the hostname from the request URL. Application host: refuse `/_render/*` with 404; pass `/api/*`, `/_assets/*`, `/login/*`, `/docs/*`, `/publish/*`, and `/invite/*` through with no session check; on every other path run the session check. The gate applies the application header set (`frame-ancestors 'none'`, `X-Frame-Options: DENY`, nosniff, referrer policy) only to responses that carry no `Content-Security-Policy` of their own, so function-originated responses from `netlify/lib/hosted/http.mjs` and `documents.mjs` keep their headers and the hosted viewer's `frame-src` survives. The gate's internal session subrequest targets `/api/hosted/session` on the application origin and therefore hits the pass-through branch; no recursion. Renderer host: rewrite exactly `/`, `/renderer.js`, `/renderer.css`, `/renderer-config.js` to `/_render/…` by internal rewrite (URL unchanged), apply the renderer header set ported from `renderer/scripts/build.mjs` (CSP with `frame-ancestors https://archon.aiur.team`, `Referrer-Policy: no-referrer`, `Permissions-Policy`, `Cross-Origin-Resource-Policy: cross-origin`, no `X-Frame-Options`), 404 every other path with no body and no `Set-Cookie`, and never call the session check. Any other host: 404 with `X-Robots-Tag: noindex`. Hostnames come from `HOSTED_APP_ORIGIN` and `HOSTED_RENDER_ORIGIN`; when both are unset every request host is the application host. Remove `excludedPath` so the gate runs on every request; the root `/*` `Cache-Control` rule stays in TOML because it is path-based and harmless on renderer paths.
...
- **Test scenarios:**
  ...
  - Edge: with no `HOSTED_*` variables set, `/` and `/<slug>/` on any host reach the application branch.
  - Integration: exactly one `Content-Security-Policy` header on every response of both hosts, including `/api/session` and `/docs/<id>` on the application host, where the function's own header is the one that survives.
...
- **Test scenarios:**
  ...
  - Edge: the issued `__Host-archon_session` cookie carries `Max-Age=86400`, and a session older than 24 hours is refused by the session route.
  - Error: destinations `/login/`, `/api/`, `/_render/` are refused by the reserved-segment rule.
...
- **Test scenarios:**
  ...
  - Error: a signed-in stranger resolves `none` on a document with no domains even when `PUBLIC_DEFAULT_ROLE=viewer` is still set in the environment.
...
- **Dependencies:** U5, U6.
- **Files:** `README.md`, `hosted/OPERATIONS.md`, `renderer/README.md`, `netlify/.env.example` (merged from `hosted/.env.example`), `scripts/test-hosted-operations.mjs` (example-file assertions point at the merged file and the `AUTH0_CLIENT_SECRET` placeholder), `docs/research/README.md` pointer.
- **Approach:** README ends with a "Hosting" section of at most twelve numbered steps: create or reuse the Netlify site, link the repository, confirm the `*.netlify.app` hostname serves, bind the custom domain, create the Auth0 application (Regular Web App) with Google and GitHub connections using your own provider apps and the `user:email` scope on GitHub, disable every other connection on the application including the default `Username-Password-Authentication` database, set callback and logout URLs, set the environment variables (`HOSTED_APP_ORIGIN`, `HOSTED_RENDER_ORIGIN`, `AUTH0_DOMAIN`, `AUTH0_CLIENT_ID`, `AUTH0_CLIENT_SECRET`, `ABLY_API_KEY`, `HOSTED_PUBLISH_ENABLED`) marked secret, deploy, run the live acceptance. OPERATIONS sections 1 to 3 describe one site and two hostnames; the renderer README explains the host-aware gate as the header authority.
- **Test scenarios:**
  - Happy path: `scripts/test-hosted-operations.mjs` asserts `netlify/.env.example` keys equal the configured key set and the `AUTH0_CLIENT_SECRET` value is a placeholder.
  - Prose is verified by review against the live setup in U13.
...
- **Approach:** Fixtures keep `https://app.archon.example.com` for the application origin and use `https://archon-example.netlify.app` for the renderer origin so the registrable-site check is exercised on the real pattern without relaxing `config.mjs`. The live runner gains: L0 the renderer hostname answers 200 and is not redirected; L0b a cookie set on the application host is absent from a renderer-host request; the host refusal matrix on both hosts; an Auth0 round trip with the operator's two test accounts (one Google, one GitHub), asserting the GitHub session carries a verified email; the login page offers exactly two providers; a domain-admit and domain-refuse pair. Live acceptance gates name only the one site, the Auth0 tenant, and the test accounts.
- **Test scenarios:**
...
- **Approach:** Operator gates: switch the Netlify site's repository to `aiur-team/archon` with the repository build settings; set the environment variables; create the Auth0 tenant, application, and both connections with own provider credentials; register callback and logout URLs; provide two test accounts. Before the switch, record the site's current repository link and build settings; trigger the first build with the cache cleared so the root `ignore` rule cannot skip it. Then run `scripts/test-hosted-live.mjs`, record the evidence, and archive `archon-docs` with a pointer only after the consolidated deploy has held green; delete it only after the operator confirms. If any live gate fails, restore the recorded repository link and build settings to return the homepage to service before retrying. Reopen the AHU-013 hold (#176) against this evidence.
- **Test scenarios:**
  - Happy path: every live gate passes and the evidence file lists the commit, hostnames, and account classes tested.
  - Error: any failed gate blocks the archive step and triggers the rollback of the site's repository link.
...
| Edge gate | `node --test netlify/edge-functions/*.test.mjs` | U2 | green |
